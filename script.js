// =====================================================================
// PhishShield AI — Detection Engine
// Core scoring logic kept identical to the original checker; only the
// output layer was rewired to feed the new dashboard elements.
// =====================================================================
...
function checkEmail() {
    ...
    let issues = [];       // { type: 'danger'|'warning'|'info', title, detail }
    let mitigation = [];
    ...
    if (email === "") {
        anomaliesList.innerHTML = "<p class='empty-hint'>Please paste an email first.</p>";
    ...
    keywords.forEach(function (word) {
        ...
        if (email.includes(word)) {
            ...
            issues.push({
                ...
                detail: "Detected imperative/social-engineering phrase: \"" + word + "\"."
            });
    ...
    // Common brand names 
    let brandTargets = ["microsoft", "google", "paypal", "apple", "amazon", "facebook", "netflix", "bank"];
    ...
    if (links) {
        ...
        links.forEach(function (link) {
            ...
            // Lightweight homograph / brand-typo heuristic
            ...
            brandTargets.forEach(function (brand) {
                ...
                if (domain && domain.includes(brand.slice(0, 4)) && !domain.includes(brand)) {
                    ...
                    issues.push({
                        ...
                        detail: "The link \"" + domain + "\" uses look-alike characters to spoof \"" + brand + "\"."
                    });
                ...
                if (domain.includes(brand) && homographPattern.test(domain)) {
                    ...
                    issues.push({
                        ...
                        icon: "🔀",
                        title: "Homograph URL Detected",
                        detail: "The link \"" + domain + "\" mixes digits into a trusted brand name."
                    });
    ...
    if (exclamation >= 3) {
        ...
        issues.push({
            ...
            detail: "Excessive exclamation marks (" + exclamation + ") suggest manufactured urgency."
        });
    ...
    if (upperWords && upperWords.length >= 3) {
        ...
        issues.push({
            ...
            detail: "Excessive capital letters detected: " + upperWords.slice(0, 3).join(", ") + "."
        });
...
flag
    ...
    let suspiciousCount = issues.filter(i => i.type === "danger" || i.type === "warning").length;

    let safeSignals = ["unsubscribe", "https", "support@", "regards", "official"];
    let safeTags = safeSignals.filter(sig => email.includes(sig)).length;

    let aiProbability = Math.min(99, Math.round(percentage * 0.9 + suspiciousCount * 1.5));
    let spamScore = Math.min(10, (score / 4)).toFixed(1);
    let knownMatch = issues.some(i => i.title === "Homograph URL Detected") ? "Spoofed Domain" : "None";
    ...
    if (issues.length === 0) {
        ...
        anomaliesList.innerHTML = "<p class='empty-hint'>No suspicious content detected.</p>";

    } else {

        anomaliesList.innerHTML = issues.map(function (item) {

            let itemClass = item.type === "danger" ? "" : item.type === "warning" ? "warning-item" : "info-item";
    ...
    document.getElementById("mitigationGrid").innerHTML = mitigation.map(function (tip) {
...
document.getElementById("clearBtn").addEventListener("click", function () {
    ...
    document.getElementById("anomaliesList").innerHTML = "<p class='empty-hint'>Run a scan to surface anomalies here.</p>";
...
document.getElementById("attachBtn").addEventListener("click", function () {
    alert("Attachment scanning isn't wired up yet — paste the email text directly for now.");
});